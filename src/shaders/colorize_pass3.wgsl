...
@group(0) @binding(1) var<storage, read> spatial_average : array<ColorizedPixel>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let input_pixel = input[index];
    let average_pixel = spatial_average[index];

    let input_color = quantize_rgb(vec3<f32>(input_pixel.r, input_pixel.g, input_pixel.b));
    let input_lab = rgb_to_lab(input_color);
    let average_lab = vec3<f32>(average_pixel.r, average_pixel.g, average_pixel.b);

    let luminance_transferred_lab = vec3<f32>(input_lab.r, average_lab.g, average_lab.b);
    ...
    output[index] = ColorizedPixel(final_color.r, final_color.g, final_color.b);
}
